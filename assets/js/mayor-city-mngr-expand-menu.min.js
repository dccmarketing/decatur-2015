...
( function( $ ) {
	...
	if ( $( 'body' ).hasClass( 'single-employee' ) && $( 'body' ).hasClass( '.city-manager' ) ) {
	...
	enquire.register( "screen and (min-width: 620px)" , {
		...
		match: function() {
			...
			if ( $( 'body' ).hasClass( 'city-manager' ) ) {

				$( 'li.city-administration' ).addClass( 'current-menu-item' );
				$( 'li.city-administration' ).addClass( 'current-menu-ancestor' );

			} /*else if ( $( 'body' ).hasClass( 'mayor' ) ) {
